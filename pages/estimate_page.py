...
# ملاحظات هامة للمطور:
# هذا الملف مخصص لصفحة تقدير التكلفة فقط. يحتوي على الركائز الأساسية (OpenAI، NLP، التفاعل بين النماذج).
...
# دالة لإضافة إدخال إلى السجل (مشترك عبر st.session_state)
...
if st.session_state["execution_log"]:
    ...
    st.text_area("سجل العمليات", log_text, height=200, key="log_text_area_estimate")
    st.download_button(
        label="تحميل سجل التنفيذ",
        data=log_text,
        file_name="execution_log.txt",
        mime="text/plain",
        key="download_log_estimate"
    )
else:
