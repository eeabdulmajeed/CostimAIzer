...
def truncate_text_to_tokens(text, max_tokens=3000, model_name="gpt-4"):
    try:
...
class CostimAIzeOrchestrator:
    def __init__(self):
        ...
        openai.api_key = os.getenv("OPENAI_API_KEY")  # تم نقل السطر إلى داخل __init__
    ...
    def understand_historical_prices(self):
        ...
        prompt = f'''
        As an experienced pricing engineer, analyze this historical pricing data: {df.to_dict(orient='records')[:50]}.
    ...
    def analyze_bid(self, sow_file, contractor_data):
        ...
        prompt = f'''
        As a cautious pricing engineer, analyze this bid: {contractor_data}.
    ...
    def estimate(self, features):
        ...
        try:
            ...
            if self.feedback_history:
                adjustment = sum(f for f in self.feedback_history if f) / len(self.feedback_history)
